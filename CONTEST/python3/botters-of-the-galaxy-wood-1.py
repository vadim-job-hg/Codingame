...
class Game:
    ...
    def collect_data(self):
        ...
        self.index += self.round_type
        entity_count = int(input())
        self.my_troops.clear()
            ...
            if (unit_type == 'HERO'):
                ...
                if int(team) == self.my_team:
                    self.my_heroes.append(
                        Hero(unit_id, team, unit_type, x, y, attack_range, health, max_health, shield, attack_damage,
                    ...
                    self.enemy_heroes.append(
                        Hero(unit_id, team, unit_type, x, y, attack_range, health, max_health, shield, attack_damage,
                             movement_speed, stun_duration, gold_value, count_down_1, count_down_2, count_down_3, mana,
                             max_mana, mana_regeneration, hero_type, is_visible, items_owned))

            if int(team) == self.my_team:
                self.my_troops.append(
    ...
    def action(self):
        if (self.round_type < 0):
            for hero in self.HEROES:
                print(hero)
        for index in range(self.round_type):
            if (self.index > 300):
    ...
    def _go_back(self):
        if self.my_team == 0 and self.my_hero.x > 1300:
            ...
            return True
        if self.my_team == 0:
            most_right = max(self.my_troops, key=lambda x: x.x)
            if not (most_right):
            ...
            most_left = min(self.my_troops, key=lambda x: x.x)
            if not (most_left):
    ...
    def need_avoid_damage(self):
        ...
        for enemy in self.enemy_units:
            print(enemy.attack_range, self.my_hero.attack_range, abs(self.my_hero.x - enemy.x), file=sys.stderr)
