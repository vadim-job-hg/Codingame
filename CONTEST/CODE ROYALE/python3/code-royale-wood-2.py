...
BUILD_ORDER = ['TOWER', 'BARRACKS-ARCHER', 'BARRACKS-KNIGHT', 'BARRACKS-GIANT', 'BARRACKS-KNIGHT', 'BARRACKS-ARCHER',
...
class Game:
    ...
    def filter_sites_by_ready(self, sites):
        return {k: v for k, v in sites.items() if v.param1 == 0}
    ...
    def _get_result(self):
        ...
        build = self.get_closest_site(sites)
        if (build):
            self._action.set_data(ValidActions.BUILD, [build.site_id, 'BARRACKS-KNIGHT'])
        else:
    ...
    def _get_train_string(self):
        ...
        for site_id, site in self.sites.items():
            if OwnerType.FRIENDLY == site.owner and StructureType.BARRACS == site.structure_type:
                can_train_ids.append(str(site_id))
        random.shuffle(can_train_ids)
        app_string = ''
        can_spend = 0
        if (len(can_train_ids) > 0):
            for train_id in can_train_ids:
                if (can_spend + 80 < self.gold):
                    can_spend += 80
