...
class Action:
    DIR_RULES = {'WEST':{'y':0,'x':-1},'EAST':{'y': 0, 'x':1},'SOUTH':{'y': 1, 'x':0},'NORTH':{'y': -1, 'x':0}}
    ...
    def __init__(self):
        ...
        if len(teleports)>1:
            self._teleports[str(teleports[0][0])+str(teleports[0][1])] = {'x':teleports[1][0], 'y':teleports[1][1]}
            self._teleports[str(teleports[1][0])+str(teleports[1][1])] = {'x':teleports[0][0], 'y':teleports[0][1]}
        print(self._bender_x, self._bender_y, file=sys.stderr)

    def is_next_wall(self):
        return self._map[self._bender_y+self.DIR_RULES[self._bender_d]['y']][self._bender_x+self.DIR_RULES[self._bender_d]['x']]=="#"

    def is_next_x(self):
        return not(self._bender_is_drunk) and self._map[self._bender_y+self.DIR_RULES[self._bender_d]['y']][self._bender_x+self.DIR_RULES[self._bender_d]['x']]=="X"
