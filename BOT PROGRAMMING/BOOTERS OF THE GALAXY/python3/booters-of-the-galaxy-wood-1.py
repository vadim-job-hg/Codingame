...
class ItemPotion(Item):
    def __init__(self, item_name, item_cost, damage, health, max_health, mana, max_mana, move_speed, mana_regeneration):
        super().__init__(item_name, item_cost)
        print(self, file=sys.stderr)

    def __str__(self):
        return "item_name: {} item_cost:{}".format(self.item_name, self.item_cost)
...
class Game:
    ...
    tactics = {
        'DEADPOOL': Tactics.DEFEND,
        'DOCTOR_STRANGE': Tactics.DEFEND,
        'HULK': Tactics.DEFEND,
        'IRONMAN': Tactics.DEFEND,
        'VALKYRIE': Tactics.DEFEND
    }
    my_team = 0
    enemy_team = 1
    neitral_team = 2
    ...
    def _append_item(self, item_name, item_cost, damage, health, max_health, mana, max_mana, move_speed,
        ...
        if (is_potion == '1'):
            self.potion_items.append(
                ItemPotion(item_name, item_cost, damage, health, max_health, mana, max_mana, move_speed,
                           mana_regeneration))
        else:
