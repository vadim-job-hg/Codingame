...
class Game:
    ...
    def _scan(self):
        ...
        self._step += 1
        if (self._step > 100):
            # self.we_need_types[0]['count']= 3
            self.we_need_types[2]['count'] = 5
    ...
    def get_closest_site(self, sites):
    ...
    def get_farest_to_creep(self, sites, creep):
        distance = 0
        build = None
        for site_id, site in sites.items():
            dis_site = dist(creep.x, creep.y, site.x, site.y)
            if (dis_site > distance):
                distance = dis_site
                build = site
        return build
    ...
    def get_closest_to(self, sites, to):
        distance = self.HEIGHT * self.WIDTH
        build = None
        for site_id, site in sites.items():
    ...
    def filter_by_distance(self, sites, coors):
        return {k: v for k, v in sites.items() if
                dist(coors['x'], coors['y'], v.x, v.y) < (self.WIDTH + self.HEIGHT) / 1.5}
    ...
    def _get_best_mine(self):
        ...
        print(sites, file=sys.stderr)
        return self.get_closest_site(sites)
    ...
    def _get_best_tower(self):
        if (self._tower_to_improve and self._tower_to_improve.param_1 < 450):
    ...
    def _get_result(self):
        ...
        if (len(creeps) > 0):
            sites = {k: v for k, v in self.sites.items() if v.structure_type != StructureType.TOWER}
            site = self.get_closest_to_queen(sites)
            if (self.my_queen.dist(site) > self.MOVE_SPEED + site.radius):
                self.move_to = site
                return True
            elif (site.structure_type != StructureType.TOWER):
                self._action.set_data(ValidActions.BUILD, [site.site_id, 'TOWER'])
                return True
